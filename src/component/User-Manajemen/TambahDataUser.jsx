...
export default function TambahDataUser() {
  ...
  const validatePassword = (value) => {
    ...
    if (!PASSWORD_REGEX.test(value)) {
      return "Kata sandi minimal 8 karakter huruf besar-kecil ";
    }
    return "";
  };
  ...
  const handleSubmit = (e) => {
    ...
    const passwordError = validatePassword(formData.password) || (formData.password === "" ? "Kata sandi wajib diisi" : "");
    const confirmPasswordError =
      validateConfirmPassword(formData.confirmPassword, formData.password) ||
      (formData.confirmPassword === "" ? "Konfirmasi kata sandi wajib diisi" : "");

    if (phoneError || emailError || passwordError || confirmPasswordError || !formData.nama || !formData.tanggalLahir || !formData.role) {
      setErrors({
    ...
    localStorage.setItem("users_data", JSON.stringify([...users, newUser]));
